...
def get_args_parser():
    ...
    # AMP
    parser.add_argument(
        "--no-amp", action="store_false", dest="amp", help="Disable FP16 training."
    )
...
def main(args):
    ...
    _log = FileLogger(
        is_master=is_main_process, is_rank0=is_main_process, output_dir=args.output_dir
    )
    _log.info(args)
    ...
    if args.standardize:
        task_mean, task_std = train_dataset.mean(args.target), train_dataset.std(
            args.target
        )
    ...
    for epoch in range(args.epochs):
        ...
        info_str = (
            "Epoch: [{epoch}] Target: [{target}] train MAE: {train_mae:.5f}, ".format(
        ...
        if model_ema is not None:
            ...
            info_str = (
                "Best EMA -- epoch={}, val MAE: {:.5f}, test MAE: {:.5f}\n".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Training equivariant networks", parents=[get_args_parser()]
    )
