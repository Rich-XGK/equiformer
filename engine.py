...
def train_one_epoch(
    ...
    for step, data in enumerate(data_loader):
        ...
        # for U, U0, G, and H these four targets,  mean == 0 and std == 1;
        # for other targets, mean == data.mean() and std == data.std().
        ...
        ):  # time.perf_counter() - wall_print > 15:
    ...
    for step, data in enumerate(data_loader):
        ...
        if step % print_freq == 0 or step == (len(data_loader) - 1):
            log_str = "[{}/{}]\tavg node: {}, ".format(
                step, len(data_loader), avg_node.avg
            )
